...
const total_pledged = GAMES_JSON.reduce( (acc, game) =>{return acc + game.pledged}, 0);

// set inner HTML using template literal
raisedCard.innerHTML = `<div>$${total_pledged.toLocaleString('en-US')}</div>`;

// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");
const number_of_games = GAMES_JSON.length;
gamesCard.innerHTML =  `<div>${number_of_games}</div>`
